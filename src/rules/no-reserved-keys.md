...
ECMAScript 3 描述了一系列的关键字和保留字，如 `if` 和 `public`，这些关键字和保留词用于或打算用于一个核心语言特性。该规范还指出，这些关键字和保留词不能作为对象属性名称，而要用字符串括起来。在 ECMAScript 3 环境下，当你在一个对象字面中使用关键字或保留字时，会发生错误。比如：
