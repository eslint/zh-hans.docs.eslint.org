...
规则可能会可选择地为其发现的违规行为提供修复方案，修复会安全地在不改变应用逻辑的情况下修正违规行为。

在使用 [`--fix` 命令行选项](./command-line-interface#--fix)或编辑器扩展时可能会自动应用修复。

在[规则页](../rules/)中可能提供修复的规则会带有 🔧 符号。

### 规则建议

规则可能会可选择地为提供建议，作为修复方案的补充或替代。建议与修复存在两点区别：

1. 建议可能改变应用程序逻辑，因此无法自动应用。
1. 建议无法通过 ESLint CLI 应用，仅可通过编辑器集成实现。

在[规则页](../rules/)中可能提供建议的规则会带有 💡 符号。
